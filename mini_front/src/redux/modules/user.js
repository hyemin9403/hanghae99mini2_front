...
const checkIdM = (username) => async (dispatch, getState) => {
  ...
  axios
    .post(`http://3.38.178.109/user/signup/checkUsername/${username}`, {
      username: username,
    })
    .then((res) => {
      console.log(res);
...
const checkEmailM = (email) => async (dispatch, getState) => {
  ...
  axios
    .post(`http://3.38.178.109/user/signup/checkEmail/${email}`, {
      email: email,
    })
    .then((res) => {
      console.log(res);
...
const actionCreators = { signupM, checkIdM, checkEmailM, loginM, logoutM };
